...
public class PlayWithFilter {
	...
	public static void main(String[] args) {
		...
		employeeList.stream().filter(employee -> employee.getSalary() > 150000L).forEach(System.out::println);

		ApplicationUtils.logHeader("filter - composed predicate - logical AND");

		Predicate<Employee> predicate1 = employee -> employee.getSalary() > 150000L;
...
class EmployeePredicate implements Predicate<Employee> {
	...
	public boolean test(Employee employee) {
		return employee.getSalary() > 150000L;
	}
}
